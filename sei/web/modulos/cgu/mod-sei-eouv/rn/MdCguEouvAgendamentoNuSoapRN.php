...
require_once dirname(__FILE__) . '/../../../../infra_php/nusoap/lib/nusoap.php';

class MdCguEouvAgendamentoNuSoapRN extends InfraRN
{
    ...
    public function executarImportacaoManifestacaoEOuv()
    {
        ...
        LogSEI::getInstance()->gravar('Teste Agendamento SEI');
        ...
        //$ultimaDataExecucao = '01/12/2015';
        $ultimaDataExecucaoFormatoEouv = $this->retornaDataFormatoEouv($ultimaDataExecucao);

        //$dataAtual = '20-12-2015';
        $dataAtualFormatoEOuv = $this->retornaDataFormatoEouv($dataAtual);

        //echo "Ultima Execucao " . $ultimaDataExecucaoFormatoEouv;
        //echo "Data Atual " . $dataAtualFormatoEOuv ;
        //exit();

        if (!@file_get_contents($urlWebServiceEOuv)) {
            throw new InfraException('Arquivo WSDL ' . $urlWebServiceEOuv . ' não encontrado.');
        }

        if (!@file_get_contents($urlWebServiceAnexosEOuv)) {
            throw new InfraException('Arquivo WSDL ' . $urlWebServiceAnexosEOuv . ' não encontrado.');
        }
        ...
        try {
            ...
            $objWS = new nusoap_client($urlWebServiceEOuv, 'wsdl',
                $proxyhost, $proxyport, $proxyusername, $proxypassword);
            $objWS->soap_defencoding = 'ISO-8859-1';
            $objWS->forceEndpoint = 'https://sistema.ouvidorias.gov.br/Servicos/ServicoManterManifestacao.svc/v1';
            ...
            $objWS = new SoapClient($urlWebServiceEOuv, array('encoding' => 'ISO-8859-1'));
        ...
        try {
            $objWSAnexo = new SoapClient($urlWebServiceAnexosEOuv, array('encoding' => 'ISO-8859-1'));
            //para uso com MTOM utilizar um componente que dê suporte a esta característica (ex.: BeSimpleSoap)
            //$objWS = new BeSimple\SoapClient\SoapClient($strWSDL, array ('encoding'=>'ISO-8859-1',
            // 'attachment_type'=>BeSimple\SoapCommon\Helper::ATTACHMENTS_TYPE_MTOM,
            // 'soap_version' => SOAP_1_1));
        ...
        $retornoWs = $objWS->GetListaManifestacaoOuvidoria(array("login" => '11111111111',
            "senha" => 'abcd1234', "numProtocolo" => '', "dataCadastroInicio" => $ultimaDataExecucaoFormatoEouv, "dataCadastroFim" => $dataAtualFormatoEOuv, "dataPrazoRespostaInicio" => '', "dataPrazoRespostaFim" => '', "situacaoManifestacao" => 1))->GetListaManifestacaoOuvidoriaResult;
        ...
        try {
            ...
            $objEouvRelatorioImportacaoDTO = $objEouvRelatorioImportacaoRN->cadastrar($objEouvRelatorioImportacaoDTO);
            ...
            if ($idUltimaExecucao != null && $idUltimaExecucao != null) {
                ...
                if ($arrProtocolosProblematicos != null) {
                    foreach ($arrProtocolosProblematicos as $objProtocolosProblematicosDTO) {
                        ...
                        $retornoWsProblematicos = $objWS->GetListaManifestacaoOuvidoria(array("login" => '11111111111',
                            "senha" => 'abcd1234', "numProtocolo" => InfraUtil::retirarFormatacao($objProtocolosProblematicosDTO->getStrProtocoloFormatado()), "dataCadastroInicio" => '', "dataCadastroFim" => '', "dataPrazoRespostaInicio" => '', "dataPrazoRespostaFim" => '', "situacaoManifestacao" => 1))->GetListaManifestacaoOuvidoriaResult;
            ...
            $objEouvRelatorioImportacaoDTO2->setNumIdRelatorioImportacao($idRelatorioImportacao);
            ...
            $objEouvRelatorioImportacaoDTO3->setNumIdRelatorioImportacao($idRelatorioImportacao);
    ...
    public function executarImportacaoLinha($retornoWsLinha){
        ...
        try {
            ...
            //echo "<br>gravou protocolo: " . $objProcedimentoDTO->getDblIdProcedimento() . "<br>";
